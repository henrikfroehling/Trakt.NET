namespace TraktNet.Objects.Tests.Basic.Json.Writer
{
    ...
    {
        [Fact]
        public void Test_CastAndCrewArrayJsonWriter_WriteArray_JsonWriter_Exceptions()
        {
        ...
        public async Task Test_CastAndCrewArrayJsonWriter_WriteArray_JsonWriter_SingleObject()
        {
            ...
            using (var jsonWriter = new JsonTextWriter(stringWriter))
            {
                var traktJsonWriter = new ArrayJsonWriter<ITraktCastAndCrew>();
                await traktJsonWriter.WriteArrayAsync(jsonWriter, traktCastAndCrews);
                stringWriter.ToString().Should().Be(@"[{""cast"":[{""character"":""Character 1"",""person"":{""name"":""Person 1"",""ids"":{""trakt"":0,""slug"":""person-1""}}}," +

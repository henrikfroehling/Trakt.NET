namespace TraktApiSharp.Tests.Objects.Get.Syncs.Watched
{
    ...
    [TestClass]
    public class TraktSyncWatchedMovieItemTests
    {
        [TestMethod]
        public void TestTraktSyncWatchedMovieItemDefaultConstructor()
        {
            var movieItem = new TraktSyncWatchedMovieItem();
        ...
        [TestMethod]
        public void TestTraktSyncWatchedMovieItemReadFromJson()
        {
            var jsonFile = TestUtility.ReadFileContents(@"Objects\Get\Syncs\Watched\SyncWatchedMovies.json");

            jsonFile.Should().NotBeNullOrEmpty();

            var movieItems = JsonConvert.DeserializeObject<IEnumerable<TraktSyncWatchedMovieItem>>(jsonFile);

namespace TraktNet.Objects.Authentication.Json.Writer
{
    ...
    internal class AuthorizationObjectJsonWriter : AObjectJsonWriter<ITraktAuthorization>
    {
        ...
        public override async Task WriteObjectAsync(JsonTextWriter jsonWriter, ITraktAuthorization obj, CancellationToken cancellationToken = default)
        {
            ...
            {
                await jsonWriter.WritePropertyNameAsync(JsonProperties.PROPERTY_NAME_ACCESS_TOKEN, cancellationToken).ConfigureAwait(false);
            ...
            {
                await jsonWriter.WritePropertyNameAsync(JsonProperties.PROPERTY_NAME_REFRESH_TOKEN, cancellationToken).ConfigureAwait(false);
                await jsonWriter.WriteValueAsync(obj.RefreshToken, cancellationToken).ConfigureAwait(false);
            }

            if (obj.Scope != null)
            {
                await jsonWriter.WritePropertyNameAsync(JsonProperties.PROPERTY_NAME_SCOPE, cancellationToken).ConfigureAwait(false);
            ...
            {
                await jsonWriter.WritePropertyNameAsync(JsonProperties.PROPERTY_NAME_TOKEN_TYPE, cancellationToken).ConfigureAwait(false);
            ...
            {
                await jsonWriter.WritePropertyNameAsync(JsonProperties.PROPERTY_NAME_IGNORE_EXPIRATION, cancellationToken).ConfigureAwait(false);

namespace TraktNet.Objects.Get.Movies.Json.Writer
{
    ...
    internal class MovieReleaseObjectJsonWriter : AObjectJsonWriter<ITraktMovieRelease>
    {
        public override async Task WriteObjectAsync(JsonTextWriter jsonWriter, ITraktMovieRelease obj, CancellationToken cancellationToken = default)
        {
            ...
            {
                await jsonWriter.WritePropertyNameAsync(JsonProperties.PROPERTY_NAME_RELEASE_DATE, cancellationToken).ConfigureAwait(false);
            ...
            {
                await jsonWriter.WritePropertyNameAsync(JsonProperties.PROPERTY_NAME_RELEASE_TYPE, cancellationToken).ConfigureAwait(false);

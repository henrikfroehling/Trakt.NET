namespace TraktApiSharp.Objects.Post.Users.HiddenItems.Responses.Json.Writer
{
    ...
    internal class UserHiddenItemsPostResponseGroupObjectJsonWriter : AObjectJsonWriter<ITraktUserHiddenItemsPostResponseGroup>
    {
        public override async Task WriteObjectAsync(JsonTextWriter jsonWriter, ITraktUserHiddenItemsPostResponseGroup obj, CancellationToken cancellationToken = default)
        {
            ...
            {
                await jsonWriter.WritePropertyNameAsync(JsonProperties.USER_HIDDEN_ITEMS_POST_RESPONSE_ADDED_GROUP_PROPERTY_NAME_MOVIES, cancellationToken).ConfigureAwait(false);
                await jsonWriter.WriteValueAsync(obj.Movies, cancellationToken).ConfigureAwait(false);
            }

            if (obj.Shows.HasValue)
            {
                await jsonWriter.WritePropertyNameAsync(JsonProperties.USER_HIDDEN_ITEMS_POST_RESPONSE_ADDED_GROUP_PROPERTY_NAME_SHOWS, cancellationToken).ConfigureAwait(false);
                await jsonWriter.WriteValueAsync(obj.Shows, cancellationToken).ConfigureAwait(false);
            }

            if (obj.Seasons.HasValue)
            {
                await jsonWriter.WritePropertyNameAsync(JsonProperties.USER_HIDDEN_ITEMS_POST_RESPONSE_ADDED_GROUP_PROPERTY_NAME_SEASONS, cancellationToken).ConfigureAwait(false);

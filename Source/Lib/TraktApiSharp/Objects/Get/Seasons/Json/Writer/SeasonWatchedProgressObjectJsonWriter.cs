namespace TraktApiSharp.Objects.Get.Seasons.Json.Writer
{
    ...
    internal class SeasonWatchedProgressObjectJsonWriter : AObjectJsonWriter<ITraktSeasonWatchedProgress>
    {
        public override async Task WriteObjectAsync(JsonTextWriter jsonWriter, ITraktSeasonWatchedProgress obj, CancellationToken cancellationToken = default)
        {
            ...
            {
                await jsonWriter.WritePropertyNameAsync(JsonProperties.SEASON_WATCHED_PROGRESS_PROPERTY_NAME_NUMBER, cancellationToken);
                await jsonWriter.WriteValueAsync(obj.Number, cancellationToken);
            }

            if (obj.Aired.HasValue)
            {
                await jsonWriter.WritePropertyNameAsync(JsonProperties.SEASON_WATCHED_PROGRESS_PROPERTY_NAME_AIRED, cancellationToken);
                await jsonWriter.WriteValueAsync(obj.Aired, cancellationToken);
            }

            if (obj.Completed.HasValue)
            {
                await jsonWriter.WritePropertyNameAsync(JsonProperties.SEASON_WATCHED_PROGRESS_PROPERTY_NAME_COMPLETED, cancellationToken);
                await jsonWriter.WriteValueAsync(obj.Completed, cancellationToken);
            }

            if (obj.Episodes != null)
            {
                var episodeWatchedProgressArrayJsonWriter = new EpisodeWatchedProgressArrayJsonWriter();
                await jsonWriter.WritePropertyNameAsync(JsonProperties.SEASON_WATCHED_PROGRESS_PROPERTY_NAME_EPISODES, cancellationToken);
                await episodeWatchedProgressArrayJsonWriter.WriteArrayAsync(jsonWriter, obj.Episodes, cancellationToken);
            }
